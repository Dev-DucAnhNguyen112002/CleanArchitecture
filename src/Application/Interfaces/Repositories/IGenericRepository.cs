...
namespace CleanArchitectureTest.Application.Interfaces.Repositories;
...
public interface IGenericRepository<T> where T : class
{
    ...
    /// <param name="disableTracking"></param>
    /// <returns></returns>
    Task<PagedResult<TResult>> GetPagedListAsync<TResult>(Expression<Func<T, bool>>? filter = null,
        Func<IQueryable<T>, IOrderedQueryable<T>>? orderBy = null,
        Func<IQueryable<T>, IIncludableQueryable<T, object>>? include = null,
        IMapper? mapper = null, int pageIndex = 1,
        int pageSize = 20, int rowModify = 0, bool disableTracking = true);

    Task<PagedResult<TResult>> GetPagedListAsync<TResult>(IQueryable<T> query,
    IMapper? mapper = null, int pageIndex = 1,
    int pageSize = 20, int rowModify = 0, bool disableTracking = true);
